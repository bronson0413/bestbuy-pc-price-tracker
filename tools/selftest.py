...
FIXTURES = [
    (
        ...
        "HP",
        "OmniBook X Flip 14",
        'HP OmniBook X Flip 14" 2-in-1, Intel Core Ultra 5 226V, 16GB RAM, 512GB SSD, Windows 11 Home',
        949.99,
    ),
    (
        "9990002",
    ...
    (
        ...
        "Dell",
        "Inspiron 14 2-in-1",
        "Dell Inspiron 14 2-in-1 Laptop, Intel Core Ultra 5 226V, 16GB, 512GB SSD, Windows 11 Home",
        979.99,
    ),
]
    ...
    with st.connect(DB) as conn:
        for sku, brand, model, title, base in FIXTURES:
            ...
            st.upsert_product(
                ...
                model_name=model,
                listing_title=title,
                url=f"https://example.invalid/{sku}",
                declared={},
            ...
            for step in range(16):  # 4 days x 4 snapshots
                ...
                obs_id = st.insert_observation(
                    ...
                    captured_at_utc=ts,
                    price_usd=price,
                    regular_price_usd=base,
                    on_sale=price < base,
                    availability="orderable",
                    source_method="synthetic_selftest",
                    source_url=None,
                    collector_version=COLLECTOR_VERSION,
                    raw={"synthetic": True},
                )
                for flag in check_price(
                    price, previous=prev["price_usd"] if prev else None
                ):
                    st.raise_flag(
                        conn,
