...
import pytest
from tracker.normalize import (normalize_cpu, normalize_ram_gb, normalize_os,
                               normalize_storage_gb, parse_spec)

HP = 'HP OmniBook X Flip 14" 2-in-1, Intel Core Ultra 5 226V, 16GB RAM, 512GB SSD, Windows 11 Home'
LENOVO = 'Lenovo Yoga 7 2-in-1 14" Intel Core Ultra 5 226V 16GB Memory 512GB SSD Win 11 Home'
...
def test_same_config_across_brands_shares_one_group():
    ...
    assert len(keys) == 1
    assert keys.pop() == "intel-ultra5-226v|16gb|512gb|windows-11-home|laptop|convertible"
...
def test_tier_key_groups_processor_classes_but_not_configurations():
    ...
    ultra5_236v = parse_spec(title=HP.replace("2-in-1", "Laptop").replace("226V", "236V"))
...
def test_tier_key_still_separates_different_processor_classes():
    ...
    ultra7 = parse_spec(title=HP.replace("2-in-1", "Laptop").replace("Ultra 5 226V", "Ultra 7 258V"))
...
def test_tier_key_still_respects_memory_and_os():
    ...
    more_ram = parse_spec(title=HP.replace("2-in-1", "Laptop").replace("16GB RAM", "32GB RAM"))
    pro = parse_spec(title=HP.replace("2-in-1", "Laptop").replace("Windows 11 Home", "Windows 11 Pro"))
