...
def run(config_path: Path, db_path: Path, mode: str, dry_run: bool = False) -> int:
    ...
    attempted = succeeded = failed = stale = 0
    ...
    with st.connect(db_path) as conn:
        ...
        for entry in products:
            ...
            # A source that re-reports an observation already on file has not
            # collected anything. The manual tier does exactly this: it replays
            # the timestamp recorded in the CSV, so every scheduled run "succeeds"
            # while the series stands still. Counting that as a success made a
            # dead collection look healthy for two days.
            ...
            if obs_id is None:
                stale += 1
                print(
                    f"  [STALE] {sku} ${quote.price_usd:.2f} via {quote.source_method} "
                    f"— already on file at {captured}, nothing new stored"
                )
                st.raise_flag(
                    conn,
                    observation_id=None,
                    sku=sku,
                    rule="no_new_observation",
                    severity="warning",
                    detail=f"{quote.source_method} re-reported the reading already "
                    f"stored at {captured}. The series did not advance.",
                )
                continue
        ...
        st.finish_run(
            ...
            succeeded=succeeded,
            failed=failed,
            notes=f"{stale} stale" if stale else "",
        )

    summary = f"\nRun complete: {succeeded}/{attempted} captured, {failed} failed"
    if stale:
        summary += f", {stale} already on file"
    print(summary + ".")

    # Exiting non-zero when nothing advanced is what makes a stalled collector
    # visible: a scheduled run that stores nothing should not report success.
    return 0 if succeeded else 1


def backfill(config_path: Path, db_path: Path) -> int:
    """Load every manual reading on file, not only the latest.

    Readings taken by hand over several days arrive as a log, not as a current
    price. The normal chain would keep only the most recent and drop the rest,
    so the history is loaded explicitly.
    """
    cfg = load_config(config_path)
    source = ManualSource()
    stored = skipped = 0
    ...
    with st.connect(db_path) as conn:
        ...
        st.finish_run(
            ...
            succeeded=stored,
            failed=0,
            notes=f"{skipped} already on file",
        )
